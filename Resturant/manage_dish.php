	...
	if(mysqli_num_rows(mysqli_query($con,$sql))>0){
		...
		if($id==''){
			if($type!='image/jpeg' && $type!='image/png'){
				...
				mysqli_query($con,"insert into dish(category_id,dish,dish_detail,status,added_on,image,type,r_id) values('$category_id','$dish','$dish_detail',1,'$added_on','$image','$food_type','$Res_Id')");
				...
				foreach($attributeArr as $key=>$val){
					...
					mysqli_query($con,"insert into dish_details(dish_id,attribute,price,status,added_on,r_id) values('$did','$attribute','$price','$status','$added_on','$Res_Id')");
			...
			if($image_error==''){
				...
				foreach($attributeArr as $key=>$val){
					...
					if(isset($dishDetailsIdArr[$key])){
						...
						mysqli_query($con,"insert into dish_details(dish_id,attribute,price,status,added_on,r_id) values('$id','$attribute','$price','$status','$added_on','$Res_Id')");
					}
